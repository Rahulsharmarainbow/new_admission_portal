...
// const DataManagerTable: React.FC = () => {
//   const { user } = useAuth();
...
//     if (!filters.academic || !filters.type) {
//       setData([]);
//       setTotal(0);
//       return;
//     }
...
//         type: filters.type,
//         s_id: user?.id,
//         academic_id: parseInt(filters.academic),
//         page: filters.page,
...
//   useEffect(() => {
//     fetchData();
//   }, [filters.page, filters.rowsPerPage, filters.order, filters.orderBy, debouncedSearch, filters.academic, filters.type]);
...
//       ...prev, 
//       academic, 
//       type: '', // Reset type when academic changes
//       page: 0 
//     }));
//   };
...
//       alert(error.response?.data?.message || 'Failed to delete data. Please try again.');
//     } finally {
...
//           disabled={!filters.academic || !filters.type}
//         >
...
//                       {!filters.academic || !filters.type 
//                         ? 'Please select Academic and Type to view data' 
//                         : filters.search 
...
//   useEffect(() => {
//     fetchData();
//   }, [filters.page, filters.rowsPerPage, filters.order, filters.orderBy, debouncedSearch, selectedAcademic, selectedType]);
...
const handleDeleteConfirm = async () => {
  ...
  try {
    ...
    const requestBody = {
      ...
      s_id: user?.id,
      // Note: The API doesn't seem to require academic_id or type based on your curl example
    };

    const response = await axios.post(
      `${apiUrl}/SuperAdmin/DataManager/Delete-Data`, // Updated endpoint name
      requestBody,
      { headers }
    );
