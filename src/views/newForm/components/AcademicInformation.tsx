...
const AcademicInformation: React.FC<AcademicInformationProps> = ({ 
  ...
  const handleSignatureSealUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      reader.onloadend = () => {
        updateFormData({ 
          signature_seal: file, // Store file object
          previewSignatureSeal: reader.result as string // Store preview URL
        });
      };
      reader.readAsDataURL(file);
    } else {
      // Clear signature seal if file input is cleared
      updateFormData({ 
        signature_seal: null,
        previewSignatureSeal: formData.academicData?.signature_seal 
          ? `${assetUrl}/${formData.academicData.signature_seal}`
          : null
      });
    }
  };
  ...
  // Get signature seal URL for display
  const getSignatureSealUrl = () => {
    // Priority 1: New uploaded preview
    if (formData.previewSignatureSeal) {
      return formData.previewSignatureSeal;
    }
    // Priority 2: Existing signature seal from API
    if (formData.academicData?.signature_seal) {
      return `${assetUrl}/${formData.academicData.signature_seal}`;
    }
    return null;
  };
  ...
  const displayNewLogoUrl = getNewLogoUrl();
  const displaySignatureUrl = getSignatureUrl();
  const displaySignatureSealUrl = getSignatureSealUrl();
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-3">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                {displaySignatureSealUrl && (
                  <div className="flex-shrink-0">
                    <img
                      src={displaySignatureSealUrl}
                      alt="Signature Seal"
                      className="w-32 h-32 object-contain rounded-lg border border-gray-300 dark:border-gray-600 bg-white p-1"
                    />
                  </div>
                ...
                <div className="flex-1 min-w-0">
                  <Label htmlFor="signature_seal" className="mb-2 block">
