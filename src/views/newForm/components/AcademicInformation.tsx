...
const AcademicInformation: React.FC<AcademicInformationProps> = ({ formData, updateFormData }) => {
  ...
  const handleSignatureUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      const file = files[0];
      updateFormData({ director_signature: file });
      
      const reader = new FileReader();
      reader.onloadend = () => {
        updateFormData({ previewSignature: reader.result as string });
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  // Get signature URL - use uploaded preview or existing signature
                ...
                {signatureUrl && (
                  <div className="flex-shrink-0">
                    <img
                      ...
                      className="w-32 h-32 object-contain rounded-lg border border-gray-300 dark:border-gray-600 bg-white"
                    />
                  </div>
                )}
                <div className="flex-1 min-w-0">
