...
//   if (type === 'demo') {
//     // 👇 For demo accounts
//     navigate(`/${user?.role}/live-accounts/edit/${accountId}`);
//   } else {
//     // 👇 For normal accounts
//     navigate(`/${user?.role}/Accounts/Edit/${accountId}`);
//   }
...
//                       {filters.search ? 'Try adjusting your search criteria' : `No ${type} accounts available`}
//                     </p>
//                     {/* Show Add button in empty state only for demo accounts */}
  ...
  const confirmDelete = async () => {
    if (accountToDelete !== null) {
      try {
        ...
        // Use POST method instead of DELETE and correct endpoint with capital D
        const response = await axios.post(
          `${apiUrl}/SuperAdmin/Accounts/Delete-accounts`, // Changed to capital D and POST method
          {
            id: [accountToDelete], // Wrap in array as API expects
            s_id: user?.id
          },
          {
            headers: {
        ...
        if (error.response?.data?.message) {
          ...
          alert('Delete failed: API endpoint not found. Please check the URL.');
        } else {
