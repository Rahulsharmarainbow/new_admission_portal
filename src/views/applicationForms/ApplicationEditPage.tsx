...
useEffect(() => {
  if (Object.keys(dynamicOptions).length > 0 && Object.keys(formData).length > 0) {
    ...
    stateFields.forEach((stateField) => {
      const targetField = getTargetFieldForState(stateField);
 ...
 // Fetch districts by state ID with better error handling
...
const fetchStates = async () => {
  try {
    ...
    if (stateOptions.length > 0) {
      ...
      return true; // Return promise for proper chaining
    }
    return false;
  } catch (error) {
...
// Replace inside fetchApplicationData()
