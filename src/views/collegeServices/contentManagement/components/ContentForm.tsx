...
//   const editorRef = useRef<any>(null);
//   const editorContentRef = useRef<string>(""); // Temporary storage for editor content
...
//   useEffect(() => {
//     if (isOpen) {
//       const initialContent = editingContent ? editingContent.html_content : "";
...
//     // Use current content for validation
//     const currentContent = editorContentRef.current || formData.html_content;
...
//     // Update form data with current editor content before submit
//     const finalContent = editorContentRef.current || formData.html_content;
...
//       setErrors(prev => ({ ...prev, html_content: "Content is required" }));
//       return;
//     }
...
//     try {
//       const payload = {
//         academic_id: updatedFormData.academic_id,
//         page_name: updatedFormData.page_name,
//         page_route: updatedFormData.page_route,
//         html_content: finalContent,
//         s_id: user?.id,
//       };
...
//   // Handle editor content change - ONLY update ref, not state
//   const handleEditorChange = (newContent: string) => {
//     editorContentRef.current = newContent;
//   };

//   // Handle editor blur - update state only when editor loses focus
...
//               <JoditEditor
//                 ref={editorRef}
//                 value={formData.html_content} // Initial value only
//                 config={editorConfig}
//                 onBlur={handleEditorBlur} // Only update state on blur
//                 onChange={handleEditorChange} // Update ref only, no state update
//               />
//             </div>
//             {errors.html_content && (
//               <p className="text-red-500 text-sm mt-1">{errors.html_content}</p>
...
const ContentForm: React.FC<ContentFormProps> = ({
  ...
  const editorRef = useRef<any>(null);
  const editorInstanceRef = useRef<any>(null); // Store editor instance

  const apiUrl = import.meta.env.VITE_API_URL;

  // Jodit Editor configuration - FIXED
  const editorConfig = {
    readonly: false,
    height: 400,
    toolbarAdaptive: false,
  ...
  // FIXED: AcademicDropdown handler
  ...
  // FIXED: Editor content change - update state immediately
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <TextInput
              ...
              color={errors.page_name ? "failure" : "gray"}
              onFocus={(e) => {
                // Ensure focus stays on text input
                e.target.select();
              }}
            />
            {errors.page_name && (
          ...
          <div>
            ...
            <div className="border border-gray-300 rounded-lg overflow-hidden">
              <JoditEditor
                ...
                value={formData.html_content}
                config={editorConfig}
                onBlur={handleEditorChange} // Update on blur
                onChange={handleEditorChange} // Also update on change
              />
            </div>
            {errors.html_content && (
              <p className="text-red-500 text-sm mt-1">{errors.html_content}</p>
