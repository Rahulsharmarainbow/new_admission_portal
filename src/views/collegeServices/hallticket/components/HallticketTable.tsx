...
// // Base64 Excel download function
...
// // Improved Base64 Excel download function
// function downloadBase64Excel(base64: string, filename: string = 'hallticket.xlsx') {
//   try {
...
//   // Handle export with base64 download
//   // const handleExport = async (id: number) => {
//   //   try {
//   //     setLoading(true);
...
//   //     const fileData = response.data.data.excel_base64;
//   //     const filename = response.data.data.filename;

//   //     downloadBase64Excel(fileData, filename);
...
//     // Response structure according to your data
//     if (response.data.success && response.data.data?.excel_base64) {
//       const fileData = response.data.data.excel_base64;
//       const filename = response.data.data.filename;
...
const HallticketTable: React.FC = () => {
  ...
  const [deleteLoading, setDeleteLoading] = useState(false);
  const [exportLoading, setExportLoading] = useState<number | null>(null); // Track which hallticket is being exported
  ...
  // Handle export with base64 download and loading state
  const handleExport = async (id: number) => {
    try {
      setExportLoading(id); // Set loading for specific hallticket
      // Don't close dropdown immediately - let user see the loading state
      ...
      setExportLoading(null); // Reset loading state
      setActiveDropdown(null); // Close dropdown after operation completes
    }
  };
                        ...
                        <td className="py-4 px-4 whitespace-nowrap text-center relative">
                          ...
                          >
                            <button
                              onClick={(e) => toggleDropdown(hallticket.id, e)}
                              className="text-gray-500 hover:text-gray-700 p-2 rounded-lg hover:bg-gray-100 transition-colors"
                              disabled={exportLoading === hallticket.id}
                            >
                              <BsThreeDotsVertical className="w-4 h-4" />
                            </button>
                            {activeDropdown === hallticket.id &&
                              createPortal(
                                ...
                                >
                                  ...
                                  <button
                                    onClick={() => handleExport(hallticket.id)}
                                    className="flex items-center justify-between w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                    disabled={exportLoading === hallticket.id}
                                  >
                                    <div className="flex items-center">
                                      <BsDownload className="w-4 h-4 mr-3" />
                                      {exportLoading === hallticket.id ? 'Exporting...' : 'Export'}
                                    </div>
                                    {exportLoading === hallticket.id && (
