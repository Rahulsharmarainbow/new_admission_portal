...
const CandidateApplicationEditPage: React.FC = () => {
  let { institute_id, id: applicationId } = useParams();
  const navigate = useNavigate();
  const { candidateUser, candidateToken, isAuthenticated } = useCandidateAuth();
  ...
  const handleInputChange = useCallback(
    ...
    [errors, apiUrl, candidateToken, formData],
  );
  ...
  const handleSelectChange = useCallback(
    ...
    [errors, apiUrl, candidateToken, formData, formSections],
  );
  ...
  const fetchApplicationData = async () => {
    ...
    try {
      const response = await axios.post<ApiResponse>(
        `${apiUrl}/Candidates/get-candidate-applications`,
        { application_id: parseInt(applicationId), s_id: candidateUser.id },
        {
          headers: {
            Authorization: `Bearer ${candidateToken}`,
            'Content-Type': 'application/json',
              ...
              try {
                ...
                if (statesResponse.data?.districts) {
                  ...
                  if (existingStateValue && stateOptions.length > 0) {
                    ...
                    if (
                      stateField?.apiurl &&
                      stateField.apiurl.includes('get_district_by_state_id') &&
                      stateField.target
                    ) {
                      ...
                      if (districtResponse.data?.districts) {
                        const districtOptions = districtResponse.data.districts.map(
                          (district: any) => ({
                        ...
                        if (existingDistrictValue && districtOptions.length > 0) {
                          ...
                          if (
                            matchingDistrict &&
                            formData[stateField.target] !== matchingDistrict.value
                          ) {
                            setFormData((prev) => ({
                              ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      submitFormData.append('customer_id', candidateUser?.id?.toString() || '');
      submitFormData.append('academic_id', candidateUser?.academic_id?.toString() || '');
      submitFormData.append('s_id', candidateUser?.id?.toString() || '');
      ...
      const response = await axios.post(
        `${apiUrl}/Candidates/Candidate-Applications-update`,
        submitFormData,
        {
          headers: {
            Authorization: `Bearer ${candidateToken}`,
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      ...
      />
      <div className="mx-auto px-8 sm:px-6 lg:px-8 mt-5">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Application Edit Page</h1>
            <p className="text-gray-600 mt-2">View and edit your applications</p>
          </div>
...
export default CandidateApplicationEditPage;
