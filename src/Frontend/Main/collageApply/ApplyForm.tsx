...
const steps = ['FILL APPLICATION', 'CONFIRM & PAY', 'SUCCESS']; // 🔥 3 steps now
...
const ApplyForm: React.FC<ApplyFormProps> = ({
  ...
  }, [paymentData, formData, academic_id, apiUrl]);
        ...
        handler: async (paymentResponse: any) => {
          try {
            ...
            if (verifyResponse.data.success) {
              ...
              setActiveStep(2); // 🔥 DIRECT TO SUCCESS STEP (now step 2)
            }
          } catch (error) {
  ...
  // 🔥 UPDATED handleNext - Now goes 0→1→2
  ...
  // 🔥 UPDATED renderStep - Removed case 2, SuccessStep now case 2
    ...
    <div className="group relative mb-8">
      {stepTransitionLoading && (
        <div className="fixed inset-0 bg-transparent bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 shadow-2xl flex flex-col items-center gap-4 max-w-md mx-4">
            <Loader />
            <p className="text-gray-700 font-semibold text-lg">Processing your payment...</p>
      ...
      <div className="relative bg-white rounded-2xl border border-gray-100 overflow-hidden">
        ...
        <div className="p-4 sm:p-6">
          ...
          <div className="flex items-center justify-center mb-6 sm:mb-8">
            <div className="flex items-center w-full max-w-3xl overflow-x-auto">
              {steps?.map((step, index) => (
                <React.Fragment key={step}>
                  <div className="flex flex-col items-center min-w-[70px] sm:min-w-0">
                    <div
                      className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center border-2 font-semibold text-sm sm:text-base transition-all duration-300 ${
                        index === activeStep
                    ...
                    >
                      {index < activeStep ? (
                        <Icon icon="solar:check-circle-line-duotone" className="w-5 h-5 sm:w-6 sm:h-6" />
                      ) : (
                        index + 1
                      )}
                    </div>
                  ...
                  {index < steps.length - 1 && (
                    <div
                      className={`flex-1 h-1 mx-2 sm:mx-3 transition-all duration-300 min-w-[24px] ${
          ...
          {/* Step Content */}
          <div className="min-h-[350px] sm:min-h-[450px]">{renderStep(activeStep)}</div>
          ...
          <div className="flex flex-col sm:flex-row justify-between gap-4 mt-8 sm:mt-10">
            {activeStep !== 2 ? (
              <button
                ...
                className={`flex items-center justify-center gap-2 px-6 py-3 rounded-xl border transition-all duration-200 order-2 sm:order-1 font-medium ${
                  activeStep === 0 || stepTransitionLoading
                    ? 'border-gray-300 text-gray-400 cursor-not-allowed'
                    : 'border-blue-600 text-blue-600 hover:bg-blue-50 hover:shadow-md hover:scale-105'
                }`}
              >
            ...
            <div className="flex gap-3 order-1 sm:order-2">
              {activeStep === 2 ? (
                <button
                  onClick={handleReset}
                  className="bg-green-600 hover:bg-green-700 text-white px-6 sm:px-8 py-3 rounded-xl font-semibold transition-all duration-200 hover:shadow-xl transform hover:scale-105 w-full sm:w-auto"
                >
                ...
                <button
                  ...
                  className={`flex items-center justify-center gap-2 px-6 sm:px-8 py-3 rounded-xl font-semibold text-white transition-all duration-200 w-full sm:w-auto ${
                    loading || stepTransitionLoading
                      ? 'bg-gray-400 cursor-not-allowed'
                      : 'bg-gradient-to-r from-blue-600 to-blue-700 hover:shadow-xl transform hover:scale-105'
                  }`}
                >
                  {loading || stepTransitionLoading ? (
                    <>
          ...
          {Object.keys(errors).length > 0 && activeStep === 0 && (
            <div className="mt-6 p-4 bg-red-50 border border-red-200 rounded-xl">
              <div className="flex items-center gap-3 text-red-700 text-sm">
                <Icon icon="solar:danger-triangle-line-duotone" className="w-5 h-5 flex-shrink-0" />
      ...
      {showPaymentDialog && (
        <div className="fixed inset-0 bg-transparent bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full mx-4 transform transition-all duration-300 scale-100 max-h-[85vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200">
              <h3 className="text-center text-xl font-bold text-green-600 mb-2">
                Confirm Payment
              </h3>
            ...
            <div className="flex flex-col sm:flex-row gap-3 p-6 border-t border-gray-200">
              <button
                ...
                className={`px-6 py-3 border border-gray-400 text-gray-700 rounded-xl font-semibold transition-colors duration-200 flex-1 ${
              ...
              <button
                ...
                disabled={paymentLoading}
                className={`px-6 py-3 rounded-xl font-semibold text-white transition-all duration-200 flex-1 ${
                  paymentLoading
                    ? 'bg-gray-400 cursor-not-allowed'
