...
const PreviewStep: React.FC<PreviewStepProps> = ({
  ...
  const renderFieldValue = (child: any) => {
    switch (child.type) {
      case 'heading':
        return (
          <div className="text-start w-full">
            <h4 className="text-base md:text-lg font-bold text-[#dc2626] inline-flex items-center">
      ...
      case 'heading2':
        return (
          <div className="w-full">
            <p className="text-sm md:text-base font-semibold text-gray-800 mb-0">
              <b>{child.content}</b>
            </p>
          </div>
        );
      ...
      case 'checkbox':
        ...
        return (
          ...
          disabled:opacity-70 disabled:bg-gray-200"
            />
            <span className="text-gray-800 text-sm md:text-base">{child.content}</span>
          </div>
        );
      ...
      case 'select':
        ...
        return <span className="text-gray-800 text-sm md:text-base">{displayText}</span>;

      case 'text':
      case 'number':
      case 'email':
      case 'tel':
        return <span className="text-gray-800 text-sm md:text-base">{formData[child.name] || 'Not provided'}</span>;

      case 'textarea':
        return (
          <span className="text-gray-800 whitespace-pre-wrap text-sm md:text-base">
      ...
      case 'date':
        ...
        if (dateValue) {
          try {
            ...
            return <span className="text-gray-800 text-sm md:text-base">{formattedDate}</span>;
          } catch {
            return <span className="text-gray-800 text-sm md:text-base">{dateValue}</span>;
          }
        }
        return <span className="text-gray-800 text-sm md:text-base">Not provided</span>;

      case 'para':
        return (
          <div className="w-full">
            <p className="text-gray-600 text-xs md:text-sm leading-relaxed">{child.content}</p>
          </div>
        );
        ...
        if (fileData[child.name]) {
          return (
            <div className="text-center space-y-2 md:space-y-3">
              <div className="bg-gray-50 p-2 md:p-3 rounded-xl border-2 border-dashed border-green-500">
                <div className="mx-auto w-12 h-12 md:w-20 md:h-20 bg-gray-100 rounded-lg flex items-center justify-center mb-1 md:mb-2 overflow-hidden">
                  <img
                ...
                <p className="text-xs text-gray-600 mb-1 md:mb-2">{child.content}</p>
                <div className="bg-gradient-to-r from-green-600 to-green-700 text-white px-2 py-1 md:px-3 md:py-1 rounded-lg text-xs font-semibold">
                  ✓ Uploaded
                </div>
              </div>
            </div>
          ...
          return (
            <div className="text-center space-y-2 md:space-y-3">
              <div className="bg-gray-50 p-2 md:p-3 rounded-xl border-2 border-dashed border-gray-300">
                <div className="mx-auto w-12 h-12 md:w-20 md:h-20 bg-gray-100 rounded-lg flex items-center justify-center mb-1 md:mb-2">
                  <Icon icon="solar:upload-line-duotone" className="w-6 h-6 md:w-8 md:h-8 text-gray-400" />
                </div>
                <p className="text-xs text-gray-600 mb-1 md:mb-2">{child.content}</p>
                <div className="bg-gradient-to-r from-gray-400 to-gray-500 text-white px-2 py-1 md:px-3 md:py-1 rounded-lg text-xs font-semibold">
                  Not Uploaded
                </div>
              </div>
            </div>
          ...
          return (
            <div className="flex flex-wrap gap-1 md:gap-2 items-center justify-center">
              {Array.from({ length: 12 }, (_, index) => (
                <div
                  key={index}
                  className="w-6 h-6 md:w-8 md:w-10 md:h-10 text-center border border-gray-300 rounded-lg bg-white flex items-center justify-center text-sm md:text-lg font-semibold text-gray-800"
                >
          ...
          return (
            <div className="flex flex-wrap gap-1 md:gap-2 items-center justify-center">
              {Array.from({ length: 12 }, (_, index) => (
                <div
                  key={index}
                  className="w-6 h-6 md:w-8 md:w-10 md:h-10 text-center border border-gray-300 rounded-lg bg-gray-100 flex items-center justify-center text-sm md:text-lg font-semibold text-gray-400"
                >
                  X
                </div>
              ))}
      ...
      case 'radio':
        ...
        return <span className="text-gray-800 text-sm md:text-base">{radioDisplayText}</span>;
  ...
  return (
    <div className="preview_form space-y-4 md:space-y-6 px-2 sm:px-4">
      {/* Header */}
      <div className="text-center px-2">
        <h2 className="text-xl md:text-2xl font-bold text-blue-600 mb-2">Application Preview</h2>
      ...
      {/* Form Data Preview */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 md:gap-6 w-full">
        {dynamicBoxes.map((box, boxIndex) => {
          ...
          return (
            <div
              key={boxIndex}
              className={`col-span-12 ${getGridCols()} bg-white rounded-xl shadow-sm border border-gray-200 p-3 md:p-6 transition-all duration-300 hover:shadow-lg hover:border-blue-300 group`}
            >
              {/* Box Header with optional icon */}
              {box.title && (
                <div className="flex items-center gap-2 md:gap-3 mb-3 md:mb-4 pb-2 md:pb-3 border-b border-gray-100">
                  {box.icon && (
                    <div className="p-1 md:p-2 bg-blue-100 rounded-lg text-blue-600">
                      <Icon icon={box.icon} className="w-4 h-4 md:w-5 md:h-5" />
                    </div>
                  )}
                  <div>
                    <h3 className="text-base md:text-lg font-semibold text-gray-800 group-hover:text-blue-700 transition-colors">
                      {box.title}
                    </h3>
                    {box.description && (
                      <p className="text-xs md:text-sm text-gray-600 mt-0.5 md:mt-1">{box.description}</p>
                    )}
                  </div>
                </div>
              )}

              {/* Box Content with responsive grid */}
              <div className="w-full">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4">
                  {box.children.filter(shouldDisplayField).map((child: any, childIndex: number) => {
                    ...
                    const getColumnSpan = () => {
                      ...
                      if (childWidthValue >= 30) return 'col-span-1';
                      if (childWidthValue >= 20) return 'col-span-1';
                      return 'col-span-1';
                    };

                    const spanClass = {
                      full: 'col-span-full',
                      'sm:col-span-2 lg:col-span-2': 'col-span-1 sm:col-span-2 lg:col-span-2',
                      'col-span-1': 'col-span-1',
                    }[getColumnSpan()] || 'col-span-1';
                        ...
                        <div
                          className={`
                            ${
                              ...
                              child.type === 'para'
                                ? ''
                                : 'min-h-10 md:min-h-12 p-2 md:p-3 border border-gray-200 rounded-lg bg-gray-50 flex items-center hover:border-blue-300 hover:bg-blue-50 transition-all duration-200'
                            }
                            ${
                              child.type === 'file_button'
                                ? 'bg-transparent border-0 p-0 hover:bg-transparent min-h-0'
                                : ''
                            }
                            ${
                              child.type === 'checkbox'
                            ...
                            ${child.type === 'adhar' ? 'min-h-16 md:min-h-20' : ''}
                          `}
                        >
      ...
      {/* Payment Summary */}
      {payableAmount > 0 && (
        <div className="bg-white rounded-xl shadow-md border-2 border-blue-200 overflow-hidden mx-2 sm:mx-0">
          <div className="p-4 md:p-6">
            <h3 className="text-center text-lg md:text-xl font-bold text-green-600 mb-3 md:mb-4">
              Payment Summary
            </h3>
                ...
                <table className="w-full min-w-[300px]">
                  <thead>
                    <tr className="bg-blue-50">
                      <th className="text-left p-3 md:p-4 font-bold text-gray-700 border-b border-gray-200 text-sm md:text-base">
                        Description
                      </th>
                      <th className="text-right p-3 md:p-4 font-bold text-gray-700 border-b border-gray-200 text-sm md:text-base">
                        Amount
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr>
                      <td className="p-3 md:p-4 font-medium text-gray-800 border-b border-gray-100 text-sm md:text-base">
                        Total Payable Fee
                      </td>
                      <td className="p-3 md:p-4 text-right font-bold text-green-600 border-b border-gray-100 text-sm md:text-base">
            ...
            <div className="mt-3 md:mt-4 p-3 md:p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              <div className="flex items-center justify-center text-yellow-700">
                <Icon icon="solar:info-circle-line-duotone" className="w-4 h-4 md:w-5 md:h-5 mr-2" />
      ...
      <div className="flex flex-col sm:flex-row justify-center gap-3 md:gap-4 mt-6 md:mt-8 px-2">
        {onEdit && (
          <button
            onClick={onEdit}
            className="flex items-center justify-center gap-2 px-4 py-2 md:px-6 md:py-3 border-2 border-blue-600 text-blue-600 rounded-lg font-semibold hover:bg-blue-50 hover:shadow-md transition-all duration-200 text-sm md:text-base w-full sm:w-auto"
          >
            <Icon icon="solar:edit-line-duotone" className="w-4 h-4 md:w-5 md:h-5" />
            Edit Application
          </button>
        )}
        ...
        {onPayment && payableAmount > 0 && (
          <button
            onClick={onPayment}
            className="flex items-center justify-center gap-2 px-4 py-2 md:px-6 md:py-3 bg-green-600 text-white rounded-lg font-semibold hover:bg-green-700 hover:shadow-lg transform hover:scale-105 transition-all duration-200 text-sm md:text-base w-full sm:w-auto"
          >
            <Icon icon="solar:card-line-duotone" className="w-4 h-4 md:w-5 md:h-5" />
