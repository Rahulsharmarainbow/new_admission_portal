  ...
  const defaultSections = {
    ...
    sections: {
      getInTouch: {
        ...
        website: footerData?.academic_website || '',
      },
      admissionEnquiries: {
        ...
        contacts: [
          {
            ...
            href: `mailto:${
              footerData?.admission?.admissio_query_email || 'info@university.edu.in'
            }`,
          },
          {
            type: 'phone',
        ...
        contacts: [
          {
            ...
            href: `mailto:${
              footerData?.technical?.technical_support_email || 'support@example.com'
            }`,
          },
          {
            type: 'phone',
  ...
  return (
    <div>
      <div className="bg-white text-gray-800 p-8 mx-4 mb-4 border-t border-gray-200">
        <h3 className="text-2xl font-bold mb-6 text-center text-red-800">{title}</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
          ...
          <div className="p-4">
            ...
            <p className="text-gray-700 leading-relaxed">{sections.getInTouch.content}</p>
            <Link
              to={sections.getInTouch.website}
              target="_blank"
              rel="noopener noreferrer"
              className="mt-5 inline-flex items-center gap-2 text-blue-600 font-medium hover:text-blue-800 transition duration-200 underline-offset-4 hover:underline"
            >
              <TbWorldLatitude className="w-4 h-4" />
              {sections.getInTouch.website}
            </Link>
          </div>
          ...
          <div className="p-4">
            ...
            {sections.admissionEnquiries.contacts.map((contact, index) => (
              <p
                key={index}
                className="text-gray-700 mb-3 flex items-center transition duration-200"
              >
                {getIcon(contact.type)}
                <a
                  href={contact.href}
                  className={`hover:${colorClasses.green.hover} transition duration-200`}
                >
                  {contact.value}
                </a>
              </p>
          ...
          <div className="p-4">
            ...
            {sections.technicalSupport.contacts.map((contact, index) => (
              ...
              >
                ...
                <a
                  href={contact.href}
                  className={`hover:${colorClasses.purple.hover} transition duration-200`}
                >
                  {contact.value}
                </a>
              </p>
